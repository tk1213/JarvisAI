...
async def main() -> None:
    print("Sprint 6 Pack E — Silent Safety Integration Gate")
    print("-" * 60)
    ...
    try:
        ...
        print(
            'Say "Hey Jarvis". '
            "After the acknowledgement, remain completely silent."
        )
        print()
        ...
        with patch.object(
            ...
            if result.transcript:
                raise RuntimeError(
                    "False transcript detected while user stayed silent."
                )

            if ask_mock.await_count != 0:
                raise RuntimeError(
                    "ConversationManager was called for silent input."
                )

        print()
        print("Silent transcript rejection: PASS")
        print("Conversation execution blocked: PASS")
        print("Sprint 6 Pack E silent integration gate: PASS")
    finally:
        await app.shutdown()
